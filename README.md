...
- [Aeon](https://github.com/aaronjmars/aeon) - Autonomous agent framework that runs coding agents inside GitHub Actions workflows without approval loops or manual intervention (🏷️ `TypeScript` `GitHub Actions` `Claude Code`).
...
- [Hephaestus](https://github.com/agentlas-ai/Hephaestus) - Python runtime for packaging and routing local coding agents and skills, with meta-agent scaffolding, local ontology files, scoped memory, and policy checks for Claude Code, Codex, and Cursor (🏷️ `Python` `Local` `Claude Code` `Codex`).
